      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-brand-dark/80 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            {/* Feedback trigger */}
            <button onClick={() => setFeedbackOpen(true)} className="p-2 text-white/30 hover:text-white transition-colors" title="Feedback">
        ...
        {open && (
          <div className="md:hidden bg-brand-card border-t border-white/10 px-4 py-4 flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
              <button onClick={() => setLocale(locale === "pt" ? "en" : "pt")} className="text-sm text-white/50">
                {locale === "pt" ? "🇬🇧 English" : "🇧🇷 Português"}
              </button>
              <button onClick={() => { setFeedbackOpen(true); setOpen(false); }} className="text-sm text-white/50 flex items-center gap-2">
